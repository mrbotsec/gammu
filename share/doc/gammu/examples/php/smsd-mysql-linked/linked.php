...
function copysms($Text,$Sender,$DateTime,$id)
{
	...
	$result = sql("insert into $new_db_table (ID,Text_Offer,Text_Category,PhoneNumber,Text_Other,DateTime) values ('$id','$offerta_richiesta','$category','$Sender','" . mysql_real_escape_string($testo) . "','$DateTime')");
...
//			echo $id2[1]."<br>";
			$result = sql("update inbox set Processed='true' WHERE ID='$id2[1]'");
			$id = $id2[3];
		} else {
			$result = sql("update inbox set Processed='true' WHERE ID='$id'");
//			echo $id;
			break;
		}
	}
