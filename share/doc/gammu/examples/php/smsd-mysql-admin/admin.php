...
//			$text = "";
			...
			if ($report == "yes") {
				sql ("insert into outbox(UpdatedInDB,InsertIntoDB,Class,DestinationNumber,TextDecoded,SendingDateTime,RelativeValidity,SenderID,DeliveryReport,Coding) VALUES(now(),now(),'$class','$number','$tresc','$datoom','$validity','$phone','yes','Default_No_Compression')");
			}
			if ($report == "no") {
				sql ("insert into outbox(UpdatedInDB,InsertIntoDB,Class,DestinationNumber,TextDecoded,SendingDateTime,RelativeValidity,SenderID,DeliveryReport,Coding) VALUES(now(),now(),'$class','$number','$tresc','$datoom','$validity','$phone','no','Default_No_Compression')");
			}
			if ($report == "default") {
				sql ("insert into outbox(UpdatedInDB,InsertIntoDB,Class,DestinationNumber,TextDecoded,SendingDateTime,RelativeValidity,SenderID,Coding) VALUES(now(),now(),'$class','$number','$tresc','$datoom','$validity','$phone','Default_No_Compression')");
			}
		}
	}
}
...
if (isset($dbpass) && isset($dbconnect) && isset($_GET['op']) && isset($_GET['dzial'])) {
	if ($_GET['op']=="del") {
		if (isset($_GET['id'])) {
			$dzial = $_GET['dzial'];
			$id = $_GET['id'];
			sql ("delete from $dzial where id='$id'");
			if ($dzial == "outbox") {
				sql ("delete from outbox_multipart where id='$id'");
			}
		} else if ($delall) {
			$dzial = $_GET['dzial'];
			sql ("delete from $dzial");
			if ($dzial == "outbox") {
				sql ("delete from outbox_multipart");
			}
		}
	}
	$_GET['op']=$dzial;
...
while ($row0 = sql_fetch_object($result0)) {
